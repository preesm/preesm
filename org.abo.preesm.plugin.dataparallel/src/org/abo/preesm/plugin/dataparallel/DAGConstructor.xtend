...
class DAGConstructor {
	...
	/**
	 * Map of instances related to its actor
	 */
	private val Map<SDFAbstractVertex, SDFAbstractVertex> instance2Actor;
	...
	/**
	 * Map of explode/implode instance to its original instances (prior to creation
	 * of explode/implode)
	 */
	private val Map<SDFAbstractVertex, SDFAbstractVertex> exImOrigInstance;
	...
	new(SDFGraph sdf, Logger logger) {
		...
		instance2Actor = newHashMap()
		actor2Instances = newHashMap()
		actor2InstancesAll = newHashMap()
		exImOrigInstance = newHashMap()
	 ...
	 */
	 protected def void createInstances() throws SDF4JException {
	 	...
	 	for(actor: inputGraph.vertexSet) {
	 		...
	 		actor2InstancesAll.put(actor, new ArrayList(instances))
	 	}
	 }
	...
	protected def void linkEdges() {
		...
		for(edge: filteredEdges) {
			...
			val sourceInstances = actor2Instances.get(edge.source)
			val targetInstances = actor2Instances.get(edge.target)
	...
	protected def void updateMaps(SDFAbstractVertex sourceInstance, SDFAbstractVertex exImInstance) {
		...
		val updatedInstances = actor2InstancesAll.get(actor)
		updatedInstances.add(exImInstance)
		actor2InstancesAll.put(actor, updatedInstances)
