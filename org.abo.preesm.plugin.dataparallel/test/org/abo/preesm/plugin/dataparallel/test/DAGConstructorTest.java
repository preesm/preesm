...
public final class DAGConstructorTest {
  ...
  public void actor2InstancesAllHasAllVertices() {
    SDFGraph sdf = ExampleGraphs.acyclicTwoActors();
    DAGConstructor dagGen = new DAGConstructor(sdf);
    Assert.assertEquals(getAllVerticesFromMaps(dagGen.getActor2InstancesAll()), dagGen.getOutputGraph().vertexSet().size());
  ...
  /**
   * Check that actor2Instances has same size as vertices set, but without implode and explode
   */
  public void actor2InstancesHasNoImplodeExplode() {
    SDFGraph sdf = ExampleGraphs.acyclicTwoActors();
    DAGConstructor dagGen = new DAGConstructor(sdf);
    Assert.assertEquals(0, getImplodeExplodeFromMaps(dagGen.getActor2Instances()));
