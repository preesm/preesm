...
/**
 * Test setup for {@link DAGOperations}
 * 
 * @author Sudeep Kanur
 */
...
class DAGOperationsTest {
	...
	public def void checkRootInstances() {
		Assert.assertEquals(rootNodeNames, 
			new DAGOperationsImpl(dagGen).rootInstances.map[node | node.name])
	}
	...
	public def void checkExitInstances() {
		Assert.assertEquals(exitNodeNames, 
			new DAGOperationsImpl(dagGen).exitInstances.map[node | node.name])
	}
	...
	public def void checkActors() {
		Assert.assertEquals(actorNames,
			new DAGOperationsImpl(dagGen).rootActors.map[node | node.name])
	}
}