...
class ScriptRunner {
	 ...
	 */
	def processGroup(ArrayList<DAGVertex> vertexs) {
	 ...
	 * Buffers between scripted actors are also marked as potentially divisible.
	 * 
				...
				for (dagVertex : newVertices) {
					...
					for (candidate : candidates) {
						if (addedVertices.contains(candidate) || newVertices.contains(candidate) ||
							...
							// Add the vertex to the group (if not already in it))
							...
							if (!isBetweenNewVertices || dagEdge.source == dagVertex) {
								...
								for (sdfEdge : dagEdge.aggregate) {
									...
									if (buffers.size != 2) {
										throw new Exception("Abnormal number of Buffer for edge " + sdfEdge)
									}

									// Match them together
									buffers.get(0).matchWith(0, buffers.get(1), 0, buffers.get(0).nbTokens)
									
									// Set them as potentially divisible 
									buffers.get(0).indivisible = false
									buffers.get(1).indivisible = false
								}
							}
						}
					}
	...
	def run() {
		...
		for (e : scriptedVertices.entrySet) {
			...
			val inputs = sdfVertex.incomingEdges.map[
				new Buffer(it, it.targetLabel, it.cons.intValue, dataTypes.get(it.dataType.toString).size)].toList
			val outputs = sdfVertex.outgoingEdges.map[
				new Buffer(it, it.sourceLabel, it.prod.intValue, dataTypes.get(it.dataType.toString).size)].toList
	...
	def runTest() {
		...
		// Retrieve buffers
		var inputs = newArrayList(new Buffer(null, "input", parameters.get("Height") * parameters.get("Width"), 1))
		inputs.forEach[interpreter.set("i_" + it.name, it)]

		var outputs = newArrayList(
			new Buffer(null, "output",
