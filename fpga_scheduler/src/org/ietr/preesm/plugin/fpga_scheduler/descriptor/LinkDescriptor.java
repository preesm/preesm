...
public class LinkDescriptor extends ComponentDescriptor {
	...
	public void addOccupiedTimeInterval(String communicationName, int startTime,
			int finishTime) {
		if (!occupiedTimeIntervals.containsKey(communicationName)) {
			occupiedTimeIntervals.put(communicationName, new TimeInterval(
					startTime, finishTime));
		} else {
			occupiedTimeIntervals.get(communicationName).setStartTime(startTime);
			occupiedTimeIntervals.get(communicationName).setFinishTime(finishTime);
		}
	}
}
