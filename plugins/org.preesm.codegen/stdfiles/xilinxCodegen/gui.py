...
def run_step3():
	print("step 3")
	retreive_values()
	cmd_step3 = f"make step3 VERSION={vitis_version} VITIS_DIR={vitis_dir} COMMON_IMAGE={common_image} TARGET={target_platform} PLATFORM_NAME={platform_name} TARGET_BUILD={target_build}"
...
def retreive_values():
	global common_image, vitis_version, vitis_dir, target_platform, target_build, build_vivavo_platform, vivado_plaftorm_path, platform_name, target_build
...
tk.Label(variable_frame, text="Vivado platform path").grid(row=7, sticky="W")
...
e_vivado_plaftorm_path = tk.Entry(variable_frame, width=40, font=defaultFont) ; e_vivado_plaftorm_path.grid(row=7, column=1, padx=padx, pady=pady, sticky="W") ; e_vivado_plaftorm_path.insert(0, default_vivado_plaftorm_path) 
