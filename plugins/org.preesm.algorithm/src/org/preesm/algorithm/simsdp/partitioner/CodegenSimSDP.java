...
public class CodegenSimSDP {
  ...
  private static final String MAIN_SIM_SDP_C = "mainSimSDP.c";

  private static final String HETEROGENEOUS_ATTIBUTE_FUNC = """
      int find_rank_by_processor_name(const char *processor_name) {
          int rank = -1;
          int size;
          MPI_Comm_size(MPI_COMM_WORLD, &size);
          char **processor_names = (char **)malloc(size * sizeof(char *));
          for (int i = 0; i < size; i++) {
              processor_names[i] = (char *) malloc(MPI_MAX_PROCESSOR_NAME * sizeof(char));
          }
          MPI_Gather(processor_name, MPI_MAX_PROCESSOR_NAME, MPI_CHAR,
                     processor_names[0], MPI_MAX_PROCESSOR_NAME, MPI_CHAR,
                     0, MPI_COMM_WORLD);
  ...
  public CodegenSimSDP(final Scenario scenario, final PiGraph topGraph, Map<Long, String> nodeNames,
    ...
    final String fileName = MAIN_SIM_SDP_C;

    final StringBuilder content = buildContent();
    PreesmIOHelper.getInstance().print(topPath, fileName, content);

    PreesmLogger.getLogger().info(() -> MAIN_SIM_SDP_C + " file print in : " + topPath);
  }
   ...
   */
  private StringBuilder globalDefinition() {
    final StringBuilder result = new StringBuilder();
    ...
    if (!isHomogeneous) {
      ...
      if (!topGraph.getExecutableActors().isEmpty() || !nodeNames.isEmpty()) {
   ...
   */
  private StringBuilder bufferDeclaration() {
    final StringBuilder result = new StringBuilder();
    ...
    for (final AbstractActor node : topGraph.getOnlyActors()) {
      ...
      for (int index = 0; index < node.getDataInputPorts().size(); index++) {
        ...
        if (portRank.containsKey(node.getName())) {
          ...
          portRank.put(node.getName(), new LinkedHashMap<>(Map.of(index, port)));
        }
      }
  ...
  private StringBuilder exeNode() {
    ...
    for (final AbstractActor node : topGraph.getOnlyActors()) {
      ...
      if (portRank.containsKey(node.getName())) {
        ...
        // Removes last comma, will cause issues if the for-loop doesn't run, which should not happen
    ...
    topGraph.getOnlyActors().stream().filter(Actor.class::isInstance).forEach(a -> result.append(free(a)));
  ...
  private StringBuilder mpiRecv(AbstractActor node) {
    final StringBuilder result = new StringBuilder();
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      final int src = Integer.parseInt(((AbstractActor) din.getFifo().getSource()).getName().replace("sub", ""));
      ...
      result.append("MPI_Recv(" + bufferName + "," + din.getExpression().evaluateAsLong() + "," + "MPI_"
          + type.toUpperCase() + "," + source + ", label, MPI_COMM_WORLD, &status);\n");
    }
    return result;
  }
  ...
  private String mpiSend(AbstractActor node) {
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      result.append("MPI_Send(" + bufferName + "," + dout.getExpression().evaluateAsLong() + "," + "MPI_"
          + type.toUpperCase() + "," + destination + " ,label, MPI_COMM_WORLD);\n");
    }
