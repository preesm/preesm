...
import org.preesm.algorithm.mapping.model.SimSDPCoreMapping;
import org.preesm.algorithm.mapping.model.SimSDPNodeMapping;
...
public class NodePartitioner {
  ...
  private void exportArchitecture() {
    int coreIDStart = 0;
    for (final SimSDPNodeMapping node : hierarchicalArchitecture) {
      ...
      for (final SimSDPCoreMapping core : node.getCores()) {
  ...
  private int fillNodeMapping() {
    ...
    for (int i = 1; i < line.length; i++) {
      ...
      final SimSDPCoreMapping newCore = MappingFactory.eINSTANCE.createSimSDPCoreMapping();
      ...
      if ((lastIndex == -1) || !column[0].equals(hierarchicalArchitecture.get(lastIndex).getNodeName())) {
        final SimSDPNodeMapping newNode = MappingFactory.eINSTANCE.createSimSDPNodeMapping();
