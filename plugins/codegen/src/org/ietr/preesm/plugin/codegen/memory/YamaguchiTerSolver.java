 ...
 * the associated cost for all vertices of the graphs. Then, the algorithm is
 * run successively with the highest costs found as a thresold value.
 * 
 ...
 */
public class YamaguchiTerSolver<V extends WeightedVertex<Integer> & Comparable<V>, E extends DefaultEdge>
		extends MaximumWeightCliqueSolver<V, E> {

	public YamaguchiTerSolver(SimpleGraph<V, E> graph) {

		super(graph);

		this.heaviestClique = new HashSet<V>();
	...
	public void Solve() {
		...
		OrderVertexSet(graphVertices, costs);

		int i = costs.size() - 1;

		while (this.heaviestClique.isEmpty()) {
			graphVertices.addAll(graph.vertexSet());
			this.heaviestClique = maxWeightClique(graphVertices, costs.get(i));
			i--;
		}
