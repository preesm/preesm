...
public class FirstFitAllocator extends MemoryAllocator {
	 ...
	 */
	private ArrayList<ArrayList<MemoryExclusionGraphNode>> lists;
	...
	public FirstFitAllocator(DirectedAcyclicGraph graph) {
		...
		lists = new ArrayList<ArrayList<MemoryExclusionGraphNode>>(nbShuffle);
	...
	public void allocateStableSetOrder() {
		...
		ArrayList<MemoryExclusionGraphNode> list = getStableSetOrderedList();
	...
	public void allocateLargestFirst() {
		...
		ArrayList<MemoryExclusionGraphNode> list = new ArrayList<MemoryExclusionGraphNode>(
	...
	public void allocate() {
		...
		for (int iter = 0; iter < nbShuffle ; iter++) {
			...
			// Store the results
			lists.add((ArrayList<MemoryExclusionGraphNode>) list.clone());
			listsSize.add(size);
		}
	 ...
	 */
	protected int allocateInOrder(ArrayList<MemoryExclusionGraphNode> vertexList) {
		...
		for (MemoryExclusionGraphNode vertex : vertexList) {
			// Get vertex neighbors
			HashSet<MemoryExclusionGraphNode> neighbors = inputExclusionGraph
	 ...
	 */
	protected ArrayList<MemoryExclusionGraphNode> getStableSetOrderedList() {
		ArrayList<MemoryExclusionGraphNode> orderedList;
		orderedList = new ArrayList<MemoryExclusionGraphNode>();

		SimpleGraph<MemoryExclusionGraphNode, DefaultEdge> inclusionGraph = inputExclusionGraph
				.getComplementary();

		while (!inclusionGraph.vertexSet().isEmpty()) {
			OstergardSolver<MemoryExclusionGraphNode, DefaultEdge> solver;
			solver = new OstergardSolver<MemoryExclusionGraphNode, DefaultEdge>(
					inclusionGraph);

			solver.solve();

			ArrayList<MemoryExclusionGraphNode> stableSet = new ArrayList<MemoryExclusionGraphNode>(
