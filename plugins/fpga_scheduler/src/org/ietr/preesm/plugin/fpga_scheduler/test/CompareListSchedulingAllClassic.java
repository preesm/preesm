...
public class CompareListSchedulingAllClassic extends CompareListScheduling {

	public CompareListSchedulingAllClassic(String algorithmFileName,
			String parameterFileName, String architectureFileName) {
		super(algorithmFileName, parameterFileName, architectureFileName);
		System.out
				.println("\n***** Compare List Scheduling All Classic (ordered by bottom-level) Begins! *****");
		...
		parse(algorithmFileName, architectureFileName, parameterFileName);
		ListSchedulingCriticalChildClassic scheduler2 = new ListSchedulingCriticalChildClassic(
				algorithm, architecture);
		testScheduler(scheduler2, algorithm, architecture);

		parse(algorithmFileName, architectureFileName, parameterFileName);
		ListSchedulingCommunicationDelayClassic scheduler3 = new ListSchedulingCommunicationDelayClassic(
				algorithm, architecture);
		testScheduler(scheduler3, algorithm, architecture);

		parse(algorithmFileName, architectureFileName, parameterFileName);
		ListSchedulingCcCdClassic scheduler4 = new ListSchedulingCcCdClassic(
				algorithm, architecture);
		...
		System.out
				.println("\n\n***** Compare List Scheduling All Classic (ordered by bottom-level) Finishes!*****");
	}
	...
	public static void main(String[] args) {
		...
		new CompareListSchedulingAllClassic(algorithmFileName,
