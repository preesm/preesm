...
CURRENT_PUBLISHED_RELEASES=$(find ${CURRENT_SITE_FOLDER} -maxdepth 1 -regextype posix-egrep -regex "${CURRENT_SITE_FOLDER}/org.preesm-[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+\$" | rev | cut -d'/' -f 1 | rev | sort -u -r | head -n ${MAX_VERSIONS_COUNT})
