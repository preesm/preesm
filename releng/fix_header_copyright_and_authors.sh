...
EXTENSION_REGEX=".*\.\(cpp\|c\|h\|java\|xcore\|xtend\)"
...
# Get all source files recursively, excluding tests resources folders

PLUGIN_SRC=$(find ./plugins/org* \( -name xcl2.cpp -o -name preesm_md5.* \) -prune -o -type f -regex ${EXTENSION_REGEX} -print)
TESTS_SRC=$(find ./tests/org* -type d -name resources -prune -o -type f -regex ${EXTENSION_REGEX} -print)

SRC_FILES="$PLUGIN_SRC $TESTS_SRC"
...
function fixFile {
	...
	for AUTHOR in $FILEAUTHORLIST; do
		...
		LINE=`echo "$FILEAUTHORLISTWITHDATES" | grep "$AUTHOR" | cut -d' ' -f2- | sort -u | tr '<' '[' | tr '>' ']' | tail -n 1`
