...
RCPTTVER=$(cd ${DIR} && cat pom.xml | grep -o "<rcptt-version>\(.*\)</rcptt-version>" | sed 's#<rcptt-version>\(.*\)</rcptt-version>#\1#g')
...
if [ ! -e "${HOME}/.m2/repository/org/eclipse/rcptt/runner/rcptt.runner/${RCPTTVER}/rcptt.runner-${RCPTTVER}.zip" ]; then
  wget ${MIRROR}/rcptt/release/${RCPTTVER}/runner/rcptt.runner-${RCPTTVER}.zip -O rcptt.runner-${RCPTTVER}.zip
