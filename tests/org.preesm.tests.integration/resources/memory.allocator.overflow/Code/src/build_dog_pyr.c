...
void ITERATOR_build_dog_pyr(int parallelismLevels,
...
#endif
  ITERATOR_generic(parallelismLevels, SIFT_nDogLayers,
...
void build_dog_pyr(int nGpyrLayers, int totSizeWithoutLayers,
  ...
  float * srcData2 = gpyr + offset_gpyr + w*h;
...
/* #endif */
  ...
  for (int i = *start_octave; i < *stop_octave; i ++) {
    ...
    for (int j = begin_layer; j < end_layer; j ++) {
      ...
      while (index ++ < (end_line - 1)*w + end_col) {
