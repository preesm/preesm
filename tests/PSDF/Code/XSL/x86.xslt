...
<xsl:stylesheet 
    ...
    <xsl:template match="sourceCode:userFunctionCall">
        <xsl:param name="curIndent"/>
        <xsl:choose>
            <xsl:when test="@sets"><xsl:value-of select="concat($curIndent,@sets,' =')"/></xsl:when></xsl:choose><xsl:value-of select="concat($curIndent,@name,'(')"/>
        <!-- adding buffers -->
        <xsl:variable name="buffers">
            <xsl:apply-templates select="sourceCode:buffer | sourceCode:subBuffer | sourceCode:constant | sourceCode:variable |sourceCode:bufferAtIndex"/>
        </xsl:variable>
        <!-- removing last coma -->
