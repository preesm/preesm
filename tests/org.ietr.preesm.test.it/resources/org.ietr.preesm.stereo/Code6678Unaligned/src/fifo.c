...
void fifoPush(void * inputBuffer, void* headBuffer, int headSize,
		void* bodyBuffer, int bodySize) {
	if (bodyBuffer != NULL) {
		memcpy(headBuffer, bodyBuffer, headSize);
		memcpy(bodyBuffer, (char *) bodyBuffer + headSize, bodySize - headSize);
		memcpy((char *) bodyBuffer + bodySize - headSize, inputBuffer,
				headSize);
	} else {
...
void fifoPop(void * outputBuffer, void* headBuffer, int headSize,
		void* bodyBuffer, int bodySize) {
