...
<xsl:stylesheet 
    ...
    <!-- includes set manually here -->
    <xsl:template name="includeSection"> 
        <xsl:value-of select="concat($curIndent,'#include &quot;../include/x86.h&quot;',$new_line)"/>
        <xsl:value-of select="$new_line"/>
    </xsl:template>
    
    <!-- Declaring thread functions prototypes, can be avoided if main() function generated -->
    ...
    <!-- Main thread code. Can be a main() function or a thread function -->
    ...
    <!-- Section of code containing buffer declaration -->
    ...
    <!-- Independant code container (block in C) executed once -->
    <xsl:template match="sourceCode:linearCodeContainer">
        <xsl:param name="curIndent"/>
        <xsl:if test="sourceCode:bufferAllocation | sourceCode:subBufferAllocation | sourceCode:variableAllocation | sourceCode:CompoundCode | sourceCode:finiteForLoop | sourceCode:userFunctionCall |  sourceCode:sendMsg | sourceCode:receiveMsg | sourceCode:specialBehavior | sourceCode:sendInit | sourceCode:receiveInit | sourceCode:Assignment">
            <xsl:value-of select="concat($curIndent,'{ // ',@comment,$new_line)"/>
            
            <xsl:apply-templates select="sourceCode:bufferAllocation | sourceCode:subBufferAllocation | sourceCode:variableAllocation | sourceCode:CompoundCode | sourceCode:finiteForLoop | sourceCode:userFunctionCall | sourceCode:sendMsg | sourceCode:receiveMsg | sourceCode:specialBehavior | sourceCode:sendInit | sourceCode:receiveInit | sourceCode:Assignment">
                <xsl:with-param name="curIndent" select="concat($curIndent,$sglIndent)"/>
            </xsl:apply-templates>
    ...
    <!-- Independant code container (block in C) executed in an infinite loop -->
    <xsl:template match="sourceCode:forLoop">
        <xsl:param name="curIndent"/>
        <xsl:if test="sourceCode:subBufferAllocation | sourceCode:variableAllocation | sourceCode:CompoundCode | sourceCode:finiteForLoop | sourceCode:userFunctionCall |  sourceCode:sendMsg | sourceCode:receiveMsg | sourceCode:specialBehavior | sourceCode:sendInit | sourceCode:receiveInit">
            <xsl:value-of select="concat($curIndent,'for(;;){ // ',@comment,$new_line)"/>
            
            <xsl:apply-templates select="sourceCode:subBufferAllocation | sourceCode:variableAllocation | sourceCode:CompoundCode | sourceCode:finiteForLoop | sourceCode:userFunctionCall | sourceCode:sendMsg | sourceCode:receiveMsg | sourceCode:specialBehavior | sourceCode:sendInit | sourceCode:receiveInit">
                <xsl:with-param name="curIndent" select="concat($curIndent,$sglIndent)"/>
            </xsl:apply-templates>
    ...
    <!-- Independant code container (block in C) executed in a finite loop -->
